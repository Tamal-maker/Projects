...
/* 5) Analyze discounts on two consecutive days.*/    --BY USING CASE STATEMENT METHOD

WITH CTE AS (SELECT Order_Date, Discount, LAG(Discount) OVER (ORDER BY Order_Date)Previous_Day_Discount FROM new_superstore) SELECT Order_Date, Discount AS Today_Discount, Previous_Day_Discount,    CASE        WHEN Discount > Previous_Day_Discount THEN 'Increase'        WHEN Discount < Previous_Day_Discount THEN 'Decrease'        ELSE 'No Change'    END AS Discount_TrendFROM CTE;


/* 6) Evaluate moving averages using the window functions.*/

--Query 2 (Default Window Size):SELECT Order_Date, Sales, AVG(Sales) OVER (ORDER BY Order_Date) AS Moving_Average FROM new_superstore;--Query 1 (Explicit Window Size)	SELECT Order_Date, Sales, AVG(Sales) OVER (ORDER BY Order_Date ROWS BETWEEN 6 PRECEDING AND CURRENT ROW) AS Moving_Average FROM new_superstore;